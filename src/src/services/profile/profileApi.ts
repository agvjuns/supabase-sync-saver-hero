...
export async function fetchOrganization(orgId: string): Promise<Organization | null> {
  try {
    ...
    toast({
      title: "Error",
      description: 'Failed to load organization: ' + (error.message || 'Unknown error'),
      variant: "destructive",
    });
    return null;
  }
...
export async function uploadProfilePicture(userId: string, file: File): Promise<string | null> {
  try {
    ...
    if (uploadError) {
      ...
      toast({
        title: "Error",
        description: `Failed to upload profile picture: ${uploadError.message || 'Unknown error'}`,
        variant: "destructive",
      });
      return null;
    }
    ...
    if (updateError) {
      ...
      toast({
        title: "Error",
        description: `Failed to update profile with new avatar: ${updateError.message || 'Unknown error'}`,
        variant: "destructive",
      });
      return null;
    }
