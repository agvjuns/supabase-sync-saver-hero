...
export const MapMarker = ({ item, isSelected, isHovered, onItemSelect }: MapMarkerProps) => {
  ...
  const formatPrice = () => {
    if (!item.price) return 'Not set';
    return `${item.currency || 'USD'} ${item.price}`;
  };

  // Truncate long location text for better display
  const formatLocation = () => {
    return item.location || 'Not set';
  };
  ...
  return (
    ...
    >
      ...
      >
        <h4>{item.name}</h4>
        <p><span>Location:</span> <span>{formatLocation()}</span></p>
        <p><span>Quantity:</span> <span>{item.quantity}</span></p>
        <p><span>Price:</span> <span>{formatPrice()}</span></p>
