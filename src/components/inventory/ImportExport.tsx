...
const ImportExport = ({ items, onImportComplete }: ImportExportProps) => {
  ...
  // Emit an event when import dialog opens/closes
  useEffect(() => {
    // Create and dispatch a custom event for the import modal state
    const event = new CustomEvent('importModalStateChange', {
      detail: { isOpen: isImportOpen }
    });
    window.dispatchEvent(event);

    return () => {
      // If component unmounts while modal is open, dispatch closed event
      if (isImportOpen) {
        const closeEvent = new CustomEvent('importModalStateChange', {
          detail: { isOpen: false }
        });
  ...
  const downloadTemplate = () => {
    try {
      ...
      toast.error(`Template download failed: ${(error as Error).message}`);
    }
  };
    ...
    <div className="flex space-x-2">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" className="flex items-center border-blue-300 hover:bg-blue-50 text-blue-600">
            <Download className="h-4 w-4 mr-1" />
            Export
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="bg-white border border-blue-100 shadow-md">
          <DropdownMenuItem onClick={() => handleExport('csv')} className="hover:bg-blue-50 text-gray-700">
            <FileText className="h-4 w-4 mr-2 text-blue-500" />
            <span>Export as CSV</span>
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => handleExport('excel')} className="hover:bg-blue-50 text-gray-700">
            <FileSpreadsheet className="h-4 w-4 mr-2 text-green-500" />
            <span>Export as Excel</span>
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => handleExport('json')} className="hover:bg-blue-50 text-gray-700">
